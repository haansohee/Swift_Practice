...
class ViewController: UIViewController {
    ...
    @IBAction func btnLampOn(_ sender: UIButton) {
        if (isLampOn == true)  // 전구가 켜져 있을 때 전구가 켜져 있다고 Alert 실행
        {
    ...
    @IBAction func btnLampOff(_ sender: UIButton) {
        if isLampOn  // 전구가 켜져 있을 경우, 전구를 끌 것인지를 묻는 Alert 실행
        {
            ...
            let cancelAciton = UIAlertAction(title: "아니오", style: UIAlertAction.Style.default, handler: nil)
            
            lampOffAlert.addAction(offAction)
            lampOffAlert.addAction(cancelAciton)
